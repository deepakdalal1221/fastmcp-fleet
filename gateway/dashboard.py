...
global tool search, category filter, recent invocations, and inline tool tryout.
"""
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .chip { padding: 5px 10px; background: #1e293b; border: 1px solid #334155; border-radius: 12px; font-size: 12px; cursor: pointer; color: #cbd5e1; }
  .chip.on { background: #6366f1; border-color: #6366f1; color: white; }
  .chip:hover:not(.on) { background: #2a3554; }
  .stats { display: flex; gap: 10px; margin-bottom: 20px; flex-wrap: wrap; }
  .stat { background: #1e293b; border-radius: 6px; padding: 10px 14px; min-width: 90px; }
  .stat b { font-size: 20px; color: #a5b4fc; display: block; }
  .stat span { color: #94a3b8; font-size: 11px; }
  .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 10px; }
  .card { background: #1e293b; border: 1px solid #334155; border-radius: 6px; padding: 12px 14px; cursor: pointer; transition: all 0.15s; position: relative; }
  ...
  .card .meta { color: #94a3b8; font-size: 11px; margin-top: 3px; }
  ...
  .card .reset-btn { position: absolute; top: 6px; right: 6px; padding: 2px 6px; font-size: 10px; background: #334155; border: 0; border-radius: 3px; color: #cbd5e1; cursor: pointer; opacity: 0.5; }
  .card:hover .reset-btn { opacity: 1; }
  .card .reset-btn:hover { background: #b91c1c; color: white; }
  .panel { margin-top: 20px; background: #1e293b; border: 1px solid #334155; border-radius: 6px; padding: 20px; }
  .panel h2 { color: #f8fafc; margin-bottom: 6px; font-size: 18px; }
  .tool { padding: 10px 14px; background: #0f172a; border-radius: 4px; margin-bottom: 6px; cursor: pointer; border: 1px solid transparent; }
  .tool:hover { background: #172033; border-color: #334155; }
  .tool.active { border-color: #6366f1; background: #24314a; }
  .tool code { color: #a5b4fc; font-weight: 600; font-family: monospace; }
  ...
  button.primary { background: #6366f1; color: white; border: 0; padding: 9px 18px; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: 500; }
  button.primary:hover { background: #4f46e5; }
  button.primary:disabled { background: #4b5563; cursor: not-allowed; }
  button.ghost { background: transparent; border: 1px solid #334155; color: #cbd5e1; padding: 8px 14px; border-radius: 4px; cursor: pointer; font-size: 12px; }
  button.ghost:hover { border-color: #6366f1; color: #a5b4fc; }
  .elapsed { color: #94a3b8; font-size: 11px; margin-left: auto; }
  .response { margin-top: 12px; background: #0f172a; border-radius: 4px; padding: 14px; overflow-x: auto; position: relative; }
  .response .copy { position: absolute; top: 8px; right: 8px; background: #1e293b; border: 0; color: #94a3b8; padding: 4px 10px; border-radius: 3px; cursor: pointer; font-size: 11px; }
  .response .copy:hover { color: #a5b4fc; }
  ...
  .history { background: #1e293b; border: 1px solid #334155; border-radius: 6px; padding: 16px; }
  .history h3 { font-size: 14px; color: #cbd5e1; margin-bottom: 10px; }
  ...
  .empty { color: #64748b; font-size: 12px; text-align: center; padding: 20px 0; }
  a { color: #a5b4fc; text-decoration: none; }
  a:hover { text-decoration: underline; }
</style>
</head><body x-data="dashboard()" x-init="init()">
  ...
  <div class="stats">
    ...
    <div class="stat"><b x-text="history.length"></b><span>recent calls</span></div>
  </div>

  <div class="layout">
    <div>
      <input class="search" placeholder="Filter servers or search 539 tools…" x-model="filter">
      <div class="chips">
      ...
      <div x-show="toolMatches.length > 0" class="panel" x-cloak>
        ...
        <template x-for="t in toolMatches.slice(0, 15)" :key="t.name">
      ...
      <div class="panel" x-show="selected && !activeTool" x-cloak>
        ...
        <p class="sub" x-text="selected ? selectedTools.length + ' tools registered on gateway' : ''"></p>
        ...
        <div x-show="selectedTools.length === 0" class="empty">no tools registered on live gateway (check make dev is running)</div>
      </div>
      ...
      <div class="panel" x-show="activeTool" x-cloak>
        ...
        <template x-for="(schema, key) in toolInputs" :key="key">
          <div class="form-row">
            <label x-text="key + (schema.required ? ' *' : '') + '  —  ' + (schema.type || 'string')"></label>
            ...
            <template x-if="schema.type !== 'array' && schema.type !== 'object'">
              <input :placeholder="schema.description || schema.type || 'string'" x-model="formValues[key]">
            </template>
          </div>
        </template>
        <div class="actions">
        ...
        <div class="response" :class="{error: response && response.isError}" x-show="response" x-cloak>
          <button class="copy" @click="copyResponse()" x-text="copied ? 'copied ✓' : 'copy'"></button>
    ...
    <div class="history">
      ...
      <div class="empty" x-show="history.length === 0">no calls yet · click a tool to try one</div>
  ...
  <script>
    function dashboard() {
      return {
        ...
        activeTool: null, toolInputs: {}, formValues: {}, invoking: false, response: null, lastElapsed: 0, copied: false,
