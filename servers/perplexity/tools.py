...
def register_tools(mcp: FastMCP) -> None:
    @mcp.tool
    async def ask(question: str, model: str | None = None) -> dict:
        """Ask (create)."""
        if is_offline():
            n = local_store.next_id(_SID, "answers")
            record = {"id": n, "question": question, "model": model}
            await local_store.put(_SID, "answers", str(n), record)
