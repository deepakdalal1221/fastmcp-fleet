...
def register_tools(mcp: FastMCP) -> None:
    @mcp.tool
    async def list_resource_groups() -> dict:
        """List resource groups (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "resource_groups")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")

    @mcp.tool
    async def list_vms(resource_group: str | None = None) -> dict:
        """List vms (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "vms")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")
