...
def _raise_for(r: httpx.Response) -> None:
    if r.status_code in (401, 403):
        raise AuthError(f"groq auth failed: HTTP {r.status_code}")
    if r.status_code == 404:
        raise NotFoundError("groq resource not found")
    if r.status_code == 429:
        raise RateLimitError("groq rate limited")
    if r.status_code >= 400:
        raise UpstreamError(f"groq HTTP {r.status_code}: {r.text[:200]}")
...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def chat_completion(
        ...
        model: Annotated[str, Field(description="model id")] = "llama-3.1-70b-versatile",
        ...
        async with make_client("groq", timeout=60.0) as c:
    ...
    async def list_models() -> dict:
        ...
        async with make_client("groq", timeout=_TIMEOUT) as c:
