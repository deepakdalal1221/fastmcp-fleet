...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def write_file(
        ...
        if len(encoded) > _MAX_WRITE_BYTES:
            raise ValidationError(
                f"content too large: {len(encoded)} bytes (max {_MAX_WRITE_BYTES})"
            )
        target = _resolve_inside(_root(), path)
    ...
    async def list_dir(
        path: Annotated[
            str, Field(description="Directory relative to FS_ROOT; use '.' for the root")
        ] = ".",
    ) -> dict:
    ...
    async def search(
        ...
        path: Annotated[
            str, Field(description="Directory relative to FS_ROOT to search from")
        ] = ".",
    ) -> dict:
