...
def register_tools(mcp: FastMCP) -> None:
    @mcp.tool
    async def search(q: str, earliest: str | None = None) -> dict:
        """Search (create)."""
        if is_offline():
            n = local_store.next_id(_SID, "searches")
            record = {"id": n, "q": q, "earliest": earliest}
            await local_store.put(_SID, "searches", str(n), record)
    ...
    async def list_saved_searches() -> dict:
        ...
        if is_offline():
            items = await local_store.list_all(_SID, "saved_searches")
