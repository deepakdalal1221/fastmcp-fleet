...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def list_gcs_buckets() -> dict:
        """List gcs buckets (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "gcs_buckets")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")

    @mcp.tool
    async def list_compute_instances(zone: str | None = None) -> dict:
        """List compute instances (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "compute_instances")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")
