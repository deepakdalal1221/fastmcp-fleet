...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def list_products() -> dict:
        """List products (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "products")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")

    @mcp.tool
    async def list_orders(status: str | None = None) -> dict:
        """List orders (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "orders")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")
    ...
    async def list_customers() -> dict:
        """List customers (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "customers")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")
