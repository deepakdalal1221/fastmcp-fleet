...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def fetch_feed(url: str) -> dict:
        """Fetch feed (get)."""
        if is_offline():
            rec = await local_store.get(_SID, "feeds", str(url))
            if rec is None:
                raise NotFoundError(f"feeds {url} not found")
            return rec
        raise ConfigError("live mode not implemented; run offline")

    @mcp.tool
    async def list_entries(feed_url: str) -> dict:
        """List entries (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "entries")
