...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def query(q: str) -> dict:
        """Query (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "streams")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")

    @mcp.tool
    async def query_range(q: str, start: str | None = None, end: str | None = None) -> dict:
        """Query range (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "streams")
            return {"items": items, "count": len(items)}
    ...
    async def list_labels() -> dict:
        """List labels (list)."""
        if is_offline():
            items = await local_store.list_all(_SID, "labels")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")
