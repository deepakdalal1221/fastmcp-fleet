from __future__ import annotations

import os
from typing import Annotated

import httpx
from fastmcp import FastMCP
from mcp_common import local_store
from mcp_common.errors import AuthError, ConfigError, NotFoundError, RateLimitError, UpstreamError
from mcp_common.http import is_offline, make_client
from pydantic import Field

_SID = "browserbase"
_TIMEOUT = 30.0
_BASE = "https://api.browserbase.com/v1"


def _env(name: str) -> str:
    v = os.environ.get(name)
    if not v:
        raise ConfigError(f"{name} is not set")
    return v


def _raise_for(r: httpx.Response) -> None:
    if r.status_code in (401, 403):
        raise AuthError(f"auth failed: HTTP {r.status_code}")
    if r.status_code == 404:
        raise NotFoundError("not found")
    if r.status_code == 429:
        raise RateLimitError("rate limited by upstream")
    if r.status_code >= 400:
        raise UpstreamError(f"HTTP {r.status_code}: {r.text[:200]}")


def register_tools(mcp: FastMCP) -> None:
    @mcp.tool
    async def create_session() -> dict:
        """Create an item (offline: writes to local store)."""
        if is_offline():
            n = local_store.next_id(_SID, "browserbase_seq")
            record = {
                "id": n,
            }
            await local_store.put(_SID, "sessions", str(n), record)
            return {"created": record}
        raise ConfigError("live mode not implemented; run offline")

    @mcp.tool
    async def navigate(
        session_id: str,
        url: str,
    ) -> dict:
        """Send an action (offline: appends record to local store)."""
        if is_offline():
            n = local_store.next_id(_SID, "browserbase_send_seq")
            record = {"id": n, "session_id": session_id, "url": url, "sent": True}
            await local_store.put(_SID, "navigations", str(n), record)
            return record
        raise ConfigError("live mode not implemented; run offline")

    @mcp.tool
    async def screenshot(
        session_id: str,
    ) -> dict:
        """Send an action (offline: appends record to local store)."""
        if is_offline():
            n = local_store.next_id(_SID, "browserbase_send_seq")
            record = {"id": n, "session_id": session_id, "sent": True}
            await local_store.put(_SID, "screenshots", str(n), record)
            return record
        raise ConfigError("live mode not implemented; run offline")
