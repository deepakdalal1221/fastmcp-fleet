...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def click(selector: str) -> dict:
        """Click (send)."""
        if is_offline():
            n = local_store.next_id(_SID, "clicks")
            record = {"id": n, "selector": selector, "sent": True}
            await local_store.put(_SID, "clicks", str(n), record)
            return record
        raise ConfigError("live mode not implemented; run offline")
    ...
    async def fill(selector: str, value: str) -> dict:
        """Fill (send)."""
        if is_offline():
            n = local_store.next_id(_SID, "fills")
            record = {"id": n, "selector": selector, "value": value, "sent": True}
            await local_store.put(_SID, "fills", str(n), record)
            return record
        raise ConfigError("live mode not implemented; run offline")
    ...
    async def extract_text(selector: str) -> dict:
        """Extract text (get)."""
        if is_offline():
            rec = await local_store.get(_SID, "extracts", str(selector))
            if rec is None:
                raise NotFoundError(f"extracts {selector} not found")
            return rec
        raise ConfigError("live mode not implemented; run offline")
