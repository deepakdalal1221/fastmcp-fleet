...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def list_containers(
        ...
        if is_offline():
            items = await local_store.list_all(_SID, "containers")
    ...
    async def list_images() -> dict:
        """List items (offline: reads local store)."""
        if is_offline():
            items = await local_store.list_all(_SID, "images")
            return {"items": items, "count": len(items)}
        raise ConfigError("live mode not implemented; run offline")
    ...
    async def inspect_container(
        ...
        if is_offline():
            rec = await local_store.get(_SID, "containers", str(container_id))
            if rec is None:
                raise NotFoundError(f"{container_id} not found")
            return rec
        raise ConfigError("live mode not implemented; run offline")
    ...
    async def logs(
        ...
        """Get an item by id (offline: reads local store)."""
        if is_offline():
            rec = await local_store.get(_SID, "logs", str(container_id))
            if rec is None:
                raise NotFoundError(f"{container_id} not found")
            return rec
        raise ConfigError("live mode not implemented; run offline")
